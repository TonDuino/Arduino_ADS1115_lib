...
ADS1115_async::ADS1115_async(byte I2Caddr,
                             byte inputSelect,
							 byte autoGainAdjust,
...
// initialization
byte ADS1115_async::begin()
{
  // check if the ADS1115 is present at the given I2C address
...
unsigned long ADS1115_async::clearAGAnibble(unsigned long pga, byte nibbleNr) 
{
	for (byte j = 0; j < 4; j++) {
...
// - read the raw value from ADS1115 and convert to 11 bit signed value
...
byte ADS1115_async::poll() 
{
  byte configMSb; // most significant byte of config register
  byte MUX;       // input select: 3 bit value retrieved from the ADS1115 config register
  byte i;
  byte j;
  ...
  result  = 0;
  result  = Wire.read() << 8;    // read first byte and shift 8 bits left into result
  result |= Wire.read();         // read second byte, shift out the last 4 bytes and add to result

//  This is for the ADS1015
//  if (result & 0x0800) {
...
//  }
  ...
  switch (PGA) {
    case 0:
      _gain = 0.0001875;  // FSR = +/- 6.144V, 187.5 uV/digit
	  _precision = 6;     // precision in decimal digits
      break;
    case 1:
      _gain = 0.000125;   // FSR = +/- 4.096V, 125 uV/digit
	  _precision = 6;     // precision in decimal digits
      break;
    case 2:
      _gain = 0.0000625;  // FSR = +/- 2.048V, 62.5 uV/digit
	  _precision = 7;     // precision in decimal digits
      break;
    case 3:
      _gain = 0.00003125; // FSR = +/- 1.024V, 31.25 uV/digit
	  _precision = 8; // precision in decimal digits
      break;
    case 4:
      _gain = 0.000015625;  //FSR = +/- 0.512V, 15.625 uV/digit
	  _precision = 9; // precision in decimal digits
      break;
    default:
      // values 5 ... 7 are the same
      _gain = 0.0000078125; //FSR = +/- 0.256V, 7.8125 uV/digit
	  _precision = 10; // precision in decimal digits
      break;
  }
...
//		Serial.println("Increase");
        if (absResult < 98304L / j) {  // 98304 = 3 * 2^15
          PGA = PGA + 1; // increase (may be done up to 5 times)
          j = 2 * j;	 // next divider factor
//		  Serial.print("  PGA:"); Serial.println(PGA);
        }
...
//	Serial.print("MUX: "); Serial.print("\t");
//	Serial.print(MUX); Serial.print("\t");
//	Serial.print("PGA: "); Serial.print("\t");
//	Serial.print(PGA); Serial.print("\t");
...
float ADS1115_async::getVoltage() 
{
...
float ADS1115_async::getGain() 
{
  return(_gain);
}

byte ADS1115_async::getPrecision() 
{
  return (_precision); 
}

// Start next conversion
byte ADS1115_async::startConversion() 
{
  ...
  // configure ADS1115 & start single shot conversion for input = MUX and gain = PGA
  Wire.beginTransmission (_I2Caddr);
  Wire.write(configReg);   // point to the config register
