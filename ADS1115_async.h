/*
  ...
  Version 1.1:
  - fixed negative value: result was 1 biot off
  - added support for ADS1115
  
  Library for reading the ADS1115 16 bit ADC in an asynchronous way.
  ...
  - selecting any of 8 possible ways of conversion:
    ...
    + auto gain adjust. For each input type, you can enable auto gain adjust. This means the
      library will automatically increase or decrease the gain of the ADS1115 amplifier, if
      possible and necessary.
*/

#ifndef ADS1115_async_h
#define ADS1115_async_h
...
class ADS1115_async {
  public:
    ADS1115_async(byte I2Caddr, byte inputSelect, byte autoGainAdjust, unsigned long setPGA);
    byte begin();
    byte poll();
    float getVoltage();
	float getGain();
